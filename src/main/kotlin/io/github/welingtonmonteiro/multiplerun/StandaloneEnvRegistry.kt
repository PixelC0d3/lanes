 ...
 * the IDE's own Play/Debug, not by a Multiple Run group. Populated by the optional Node env-file
 * module ([io.github.welingtonmonteiro.multiplerun.nodejs.MultiplerunNodeEnvFileExtension]) so the
 * Multiple Run Monitor can show, in its Env column, the active `.env` file name (when one is loaded)
 * and let the user open the variables actually loaded into the app.
 *
 * Deliberately separate from [MultiplerunProcessRegistry]: a standalone app must keep its own run
