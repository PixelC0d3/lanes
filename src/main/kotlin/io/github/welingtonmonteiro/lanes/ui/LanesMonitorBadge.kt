...
/**
 * Shows the number of running applications as a badge on the "Lanes Monitor" tool window
...
object LanesMonitorBadge {
    ...
    fun refresh(project: Project) {
        ApplicationManager.getApplication().invokeLater {
            ...
            val toolWindow = ToolWindowManager.getInstance(project)
                .getToolWindow(LanesMonitorToolWindowFactory.TOOL_WINDOW_ID) ?: return@invokeLater
            val count = runningCount(project)
            toolWindow.setIcon(if (count <= 0) LanesIcons.OpenDashboard else countBadgeIcon(count))
        }
    }
    ...
    private fun countBadgeIcon(count: Int): Icon {
        val base = LanesIcons.OpenDashboard
...
class LanesMonitorBadgeActivity : ProjectActivity {
    override suspend fun execute(project: Project) {
        project.messageBus.connect().subscribe(
            ExecutionManager.EXECUTION_TOPIC,
            object : ExecutionListener {
                override fun processStarted(executorId: String, env: ExecutionEnvironment, handler: ProcessHandler) =
                    LanesMonitorBadge.refresh(project)
