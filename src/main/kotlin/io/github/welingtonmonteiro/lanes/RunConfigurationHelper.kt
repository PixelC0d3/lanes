...
class RunConfigurationHelper private constructor() {
    ...
    companion object {
        ...
        @JvmStatic
        fun containsLoopies(configuration: LanesRunConfiguration, target: LanesRunConfiguration): Boolean {
            ...
            for (each in configuration.getRunConfigurations()) {
                ...
                if (each is LanesRunConfiguration && containsLoopies(each, target)) {
                    return true
                }
            }
            return false
         ...
         * 130/137/143 are SIGINT/SIGKILL/SIGTERM - what the IDE stop button, Stop Lanes and
        ...
        fun withEnvFile(envData: EnvironmentVariablesData, envFilePath: String?, project: Project?): EnvironmentVariablesData {
            ...
            return try {
                ...
                LOG.info("Lanes env file '$file' loaded, keys=${fileVars.keys}")
                if (fileVars.isEmpty()) {
                    return envData
                }
                ...
                LOG.warn("Lanes: cannot read env file '$file', continuing without it", e)
                envData
            }
        }
        ...
        fun withAppEnvFile(envData: EnvironmentVariablesData, envFilePath: String?, project: Project?): EnvironmentVariablesData {
            ...
            return try {
                ...
                LOG.info("Lanes per-app env file '$file' loaded, keys=${fileVars.keys}")
                if (fileVars.isEmpty()) {
                    return envData
                }
                val merged = LinkedHashMap(envData.getEnvs())
                ...
                LOG.warn("Lanes: cannot read per-app env file '$file', continuing without it", e)
                envData
            }
        }
        ...
        fun applySaveOutput(configuration: RunConfiguration, directory: String, configurationName: String): Boolean {
            if (configuration !is RunConfigurationBase<*>) {
                LOG.warn("Lanes save console for '$configurationName': configuration type does not support output files, skipping")
                return false
            }
        ...
        /**
         * Returns a copy of the configuration with the Lanes environment variables applied on top of its own
         * (Lanes values win on conflicts). The original configuration is never modified. When the override is
        ...
        fun withEnvironmentOverride(configuration: RunConfiguration, override: EnvironmentVariablesData): RunConfiguration {
            ...
            val logPrefix = "Lanes env override for '${configuration.getName()}' (${configuration.javaClass.simpleName}): "
            ...
            if (configuration is LanesRunConfiguration) {
                // propagate to nested Lanes configurations; their own runner state applies it to their children
                val clone = configuration.clone() as LanesRunConfiguration
                clone.setEnvData(mergeEnvData(clone.getEnvData(), override))
                LOG.info(logPrefix + "propagated to nested Lanes, keys=" + clone.getEnvData().getEnvs().keys)
                return clone
            }
