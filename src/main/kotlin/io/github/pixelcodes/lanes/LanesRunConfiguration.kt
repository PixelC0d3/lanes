...
import io.github.pixelcodes.lanes.ui.LanesRunConfigurationEditor

class LanesRunConfiguration(project: Project, factory: ConfigurationFactory, name: String) :
    ...
    override fun getConfigurationEditor(): SettingsEditor<out RunConfiguration> {
        return LanesRunConfigurationEditor(getProject())
    }

    /**
     * The "configured" mark for every saved Lanes instance. `RunConfiguration.getIcon()` is
     ...
     * The type-level icon (the "Lanes" category node in that same tree, and the entry in
     * "Add New Configuration") comes from `LanesConfigurationType.getIcon()` instead and is
     ...
     */
    override fun getIcon(): Icon = LanesIcons.Configured
    ...
    override fun getState(executor: Executor, executionEnvironment: ExecutionEnvironment): RunProfileState {
        ...
        return LanesRunnerState(enabled, startOneByOne, delayTime,
     ...
     * (`restartRunning` and `startOneByOne` are forced off). Used by the Lanes
     ...
     */
    fun createStateForApps(apps: List<RunConfiguration>): LanesRunnerState {
        return LanesRunnerState(apps, false, delayTime,
                                   reuseTabs, reuseTabsWithFailure,
                                   markFailedProcess, hideSuccessProcess, envData, envFilePath,
                                   saveOutputDir, getMemoryLimits(), getReadyConditions(),
                                   getAppEnvFiles(),
    ...
    companion object {
        ...
        private fun typeMatches(configuration: RunConfiguration, saved: RunConfigurationInternal): Boolean {
            ...
            // display names that were later rebranded (e.g. "Multirun" -> "Lanes") even
            // though the type id itself changed too (from "Multirun" to "Lanes") once the
